...
"use strict"; 
    ...
    $.fn.bmdIframe = function( options ) {
        ...
        // se si chiude la modale resettiamo i dati dell'iframe per impedire ad un video di continuare a riprodursi anche quando la modale 챔 chiusa
...
function Increase(cartNo){
	var eqId="quantity"+cartNo;	
	var epId="price"+cartNo;		
	var etId="totprice"+cartNo;		
			
	
	var x = document.getElementById(eqId).value;//lay gia tri cu trong text	
	var y = document.getElementById(epId).value;//lay gia tri cu trong text	
	var t = document.getElementById(etId).value;//lay gia tri cu trong text	
	document.getElementById(cartNo).value=cartNo+"-"+x;//lay gia tri cu trong text
	
		document.getElementById(eqId).value = parseInt(x, 10) +1;// + gia tri lay dc len 1 roi gan kq vao o text
		document.getElementById(epId).value = parseInt(y,10);// + gia tri lay dc len 1 roi gan kq vao o text
		document.getElementById(etId).value = parseInt(t,10)+parseInt(y,10);// + gia tri lay dc len 1 roi gan kq vao o text
	
	 
	cart_total();
}
function Decrease(cartNo){
	var eqId="quantity"+cartNo;
	var cartNo = cartNo;
	var epId="price"+cartNo;
	var etId="totprice"+cartNo;
	...
	var x = document.getElementById(eqId).value;
    var y = document.getElementById(epId).value;//lay gia tri cu trong text	
    var t = document.getElementById(etId).value;//lay gia tri cu trong text
    document.getElementById(cartNo).value=cartNo+"-"+x;//lay gia tri cu trong text
    
    if(parseInt(x, 10) > 1){
        document.getElementById(eqId).value = parseInt(x, 10) -1;
        document.getElementById(epId).value = parseInt(y,10);// + gia tri lay dc len 1 roi gan kq vao o text
		document.getElementById(etId).value = parseInt(t,10)-parseInt(y,10);// + gia tri lay dc len 1 roi gan kq vao o text
...
function cart_total(){
	var totpriceNodeList = document.getElementsByName('totprice');
	var totprice=0;
	for (var i = 0; i < totpriceNodeList.length; i++) {
		var tot_priceElement = totpriceNodeList.item(i);
		totprice+=parseInt(tot_priceElement.value);
	}
	document.querySelector('.woocommerce-Price').innerHTML=totprice;
	
}

function Increase2(){
    var y = document.getElementById("quantity").value;//lay gia tri cu trong text
    if(parseInt(y, 10) > 0){
        document.getElementById("quantity").value = parseInt(y, 10) +1;// + gia tri lay dc len 1 roi gan kq vao o text
    }
}
function Decrease2(){
    var y = document.getElementById("quantity").value;
    if(parseInt(y, 10) > 1){
        document.getElementById("quantity").value = parseInt(y, 10) -1;
    }
}
