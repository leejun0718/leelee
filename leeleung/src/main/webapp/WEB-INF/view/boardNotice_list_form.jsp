...
<script type="text/javascript">
	function boardnnotice_insert_form() {

		document.search.action = './boardnotice_insert_form.do';
			...
			<div class="container">
				<div class="two-columns-content">
					...
					<div class="row">
						...
						<h3
							style="width: 100%; text-align: center; padding: 20px 0 40px 0;">공지사항</h3>
						<table class="board" border="1">
							...
							<c:forEach var="boardNotice" items="${boardListPage.list}">
								<tbody>
									<tr>
										<td class="boardNo"><span>${boardNotice.noticeno}</span>
										</td>
										<td class="boardTitle"><a
											href="boardNoticeView.do?noticeno=${boardNotice.noticeno}">
												${boardNotice.noticetitle} </a> <span class="boardComment">&nbsp;&nbsp;
												[댓글수] </span></td>
										<td class="boardWriter">${boardNotice.noticewriter}</td>
										<td class="boardDate">${boardNotice.noticedate}</td>
										<td class="boardInquiry">${boardNotice.noticeinquiry}</td>
									</tr>
								</tbody>
							</c:forEach>
							...
							<tfoot>
								<tr>
									<td align="center" colspan="5"><a
										href="boardNoticeList.do?pageno=1"> ◀◀ </a>&nbsp; <a
										href="boardNoticeList.do?pageno=${boardListPage.getPreviousGroupStartPageNo()}">
											◀ </a>&nbsp;&nbsp; <c:forEach
											begin="${boardListPage.startPageNo}"
											end="${boardListPage.endPageNo}" varStatus="i">
											<a
												href="boardNoticeList.do?pageno=${i.index}&searchOption=${searchOption}&keyword=${keyword}">
												<strong> ${i.index} </strong>
											</a>
										</c:forEach> <c:if test="${boardListPage.isShowNextGroup()}">
											<a
												href="boardNoticeList.do?pageno=${boardListPage.getNextGroupStartPageNo()}">
												▶ </a>&nbsp;&nbsp;
				 						</c:if> <c:if test="${boardListPage.isShowLast()}">
											<a
												href="boardNoticeList.do?pageno=${boardListPage.getTotalPageCount()}">
												▶▶ </a>&nbsp;&nbsp;
				 						</c:if></td>
								</tr>
								<tr>
									<form action="boardNoticeList.do" id="form" name="search"
										method="post" align="center">

										<select name="searchOption" size="1">
											<option id="all" value="all"
												<c:out value="${map.searchOption == 'all'?'selected':''}"/>>제목+이름+제목</option>
											<option id="noticewriter" value=noticewriter
												<c:out value="${map.searchOption == 'noticewriter'?'selected':''}"/>>이름</option>
											<option id="noticecontent" value="noticecontent"
												<c:out value="${map.searchOption == 'noticecontent'?'selected':''}"/>>내용</option>
											<option id="noticetitle" value="noticetitle"
												<c:out value="${map.searchOption == 'noticetitle'?'selected':''}"/>>제목</option>
										</select> <input type="text" name="keyword" value="${map.keyword}">
										<input type="submit" value="조회"> <input type="button"
											value="글쓰기" onclick=" boardnnotice_insert_form()">
									</form>
								</tr>
							</tfoot>
						</table>
