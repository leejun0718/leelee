...
<script type="text/javascript">
	...
	function boardqna_insert_form() {
		document.search.action = './boardQna_insert_form.do';
		...
		<section class="two-columns-section standard-grid-section section-box">
			<div class="container">
				<div class="two-columns-content">
					...
					<div class="row">
						<h3
							style="width: 100%; text-align: center; padding: 20px 0 40px 0;">문의게시판</h3>
						<table class="board" border="1">
							...
							<c:forEach var="boardQna" items="${boardListPage.list}">
								<tbody>
									<tr>
										<td class="boardNo"><span>${boardQna.qnaNo}</span></td>
										<td class="boardTitle"><a
											href="boardQna_view.do?qnaNo=${boardQna.qnaNo}">
												${boardQna.qnaTitle} </a> <span class="boardComment">&nbsp;&nbsp;
												[댓글수] </span></td>
										<td class="boardWriter">${boardQna.qnaWriter}</td>
										<td class="boardDate">${boardQna.qnaDate}</td>
										<td class="boardInquiry">${boardQna.qnaInquiry}</td>
									</tr>
								</tbody>
							</c:forEach>
							...
							<tfoot>
								<tr>
									<td align="center" colspan="5"><a
										href="./boardQnalist.do?pageno=1"> ◀◀ </a>&nbsp; <a
										href="boardQnalist.do?pageno=${boardListPage.getPreviousGroupStartPageNo()}">
											◀ </a>&nbsp;&nbsp; <c:forEach
											begin="${boardListPage.startPageNo}"
											end="${boardListPage.endPageNo}" varStatus="i">
											<a href="boardQnalist.do?pageno=${i.index}"> <strong>
													${i.index} </strong>
											</a>
										</c:forEach> <c:if test="${boardListPage.isShowNextGroup()}">
											<a
												href="boardQnalist.do?pageno=${boardListPage.getNextGroupStartPageNo()}">
												▶ </a>&nbsp;&nbsp;
				 						</c:if> <c:if test="${boardListPage.isShowLast()}">
											<a
												href="boardQnalist.do?pageno=${boardListPage.getTotalPageCount()}">
												▶▶ </a>&nbsp;&nbsp;
				 						</c:if></td>
								</tr>
								<tr>
									<form action="boardQnalist.do" id="form" name="search"
										method="post" align="center">
										<select name="searchOption" size="1">
											<option id="all" value="all"
												<c:out value="${map.searchOption == 'all'?'selected':''}"/>>제목+이름+제목</option>
											<option id="qnaWriter" value=qnaWriter
												<c:out value="${map.searchOption == 'qnaWriter'?'selected':''}"/>>이름</option>
											<option id="qnaContent" value="qnaContent"
												<c:out value="${map.searchOption == 'qnaContent'?'selected':''}"/>>내용</option>
											<option id="qnaTitle" value="qnaTitle"
												<c:out value="${map.searchOption == 'qnaTitle'?'selected':''}"/>>제목</option>
										</select>
										<input type="text" name="keyword" value="${map.keyword}">
										<input type="submit" value="조회">
										<input type="button" value="글쓰기" onclick="boardqna_insert_form()">
									</form>
								</tr>
							</tfoot>
						</table>
